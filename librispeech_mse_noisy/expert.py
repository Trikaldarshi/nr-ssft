...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert,
        ...
        self.train_dataset = OOD_Data(root = self.datarc['path'], wav_path = self.datarc['wav_path'])
        ...
        # define mse loss with reduction as sum
        self.objective1 = nn.MSELoss(reduction='sum')
        self.objective2 = IDMContrastiveLoss(sigma=self.sigma, margin=self.margin)
        # self.objective2 = LAV(sigma=self.sigma, margin=self.margin)
